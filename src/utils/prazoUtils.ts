...
//
// Regra de negócio do Capítulo 6 do PRD:
// "prazo interno = 3 dias úteis antes da licitação, às 18h".
// Centralizado aqui para não duplicar a lógica em cada tela que precisar
// exibir ou validar o prazo.
...
/**
 * Calcula o prazo interno da Salutti para uma licitação: 3 dias úteis antes
 * da data/hora da sessão, fixado às 18h.
 */
export function calcularPrazoInterno(dataAberturaSessaoISO: string): Date {
  const sessao = new Date(dataAberturaSessaoISO);
  const prazo = subtrairDiasUteis(sessao, 3);
